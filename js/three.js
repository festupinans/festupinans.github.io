...
const posArray2 = new Float32Array(particles2 * 3);

for (let i = 0; i < particles2 * 3; i++) {
  posArray2[i] = (Math.random() - 0.5) * (Math.random() * 5);
}
...
scene.add(particlesMesh, particlesMesh2);
...
const tick = () => {
  ...
  particlesMesh2.rotation.y = -0.1 * elapsedTime;

  if (mouseX > 0) {
    particlesMesh.rotation.x = mouseY * (elapsedTime * 0.00005);
    particlesMesh.rotation.y = mouseX * (elapsedTime * 0.00005);

    particlesMesh2.rotation.x = -mouseY * (elapsedTime * 0.00005);
    particlesMesh2.rotation.y = -mouseX * (elapsedTime * 0.00005);
  }
